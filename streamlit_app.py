...
# --- 🚀 SISTEMA DE MEMÓRIA COMPARTILHADA ENTRE ABAS (COOKIES & DISCO) ---
# Força a gravação de um Cookie limpo na janela principal (acessível por todas as abas)
def injetar_sincronizador_abas():
    js_sincronizador = """
    <script>
    const parentDoc = window.parent.document;
    
    // Função para ler cookies da janela principal
    function lerCookieAbas(name) {
        let nameEQ = name + "=";
    ...
    // Verifica se existe login ativo guardado no navegador e avisa o servidor
    let contaSalva = lerCookieAbas("clicker_shared_session");
    if (contaSalva) {
        window.parent.postMessage({type: "AUTO_LOGIN_REQUEST", data: contaSalva}, "*");
    }
    </script>
    """
    components.html(js_sincronizador, height=0, width=0)

# Inicialização de Variáveis de Controle no Session State
if "logado" not in st.session_state:
    st.session_state.logado = False
if "nome_usuario" not in st.session_state:
    st.session_state.nome_usuario = ""

# Executa o sincronizador nativo de abas
injetar_sincronizador_abas()
...
if not st.session_state.logado:
    ...
    with aba_login:
        ...
        if st.button("Entrar", use_container_width=True):
            ...
            if user_key in usuarios and usuarios[user_key]["senha"] == log_pass:
                ...
                # Injeta a conta de forma definitiva no Cookie raiz do navegador (compartilhado entre abas)
                cookie_val = f"{st.session_state.nome_usuario}|||{log_pass}"
                components.html(f"""
                <script>
                window.parent.document.cookie = "clicker_shared_session={cookie_val}; path=/; max-age=31536000; SameSite=Strict";
                </script>
                """, height=0, width=0)
    ...
    with aba_salvas:
        st.subheader("Contas detectadas neste navegador:")
        
        # Puxa a lista completa de contas registradas no servidor global do jogo
        usuarios_para_listar = carregar_todos_usuarios()
        
        if usuarios_para_listar:
            opcoes_contas = [bloco.get("nome_exibicao") for bloco in usuarios_para_listar.values() if "nome_exibicao" in bloco]
            opcoes_contas = list(set(opcoes_contas))
            
            conta_selecionada = st.selectbox("Escolha uma das contas do dispositivo:", opcoes_contas)
            ...
            if col_entrar.button("Confirmar e Entrar", type="primary", use_container_width=True):
                if key_selecionada in usuarios_para_listar:
                    senha_salva = usuarios_para_listar[key_selecionada]["senha"]
                    
                    if senha_confirmacao == senha_salva:
                        dados = usuarios_para_listar[key_selecionada]["dados"]
                        st.session_state.pontos = dados.get("pontos", 0)
                        st.session_state.poder_base = dados.get("poder_base", 1)
                        st.session_state.pontos_por_segundo = dados.get("pontos_por_segundo", 0)
                        st.session_state.pet_slot_1 = dados.get("pet_slot_1", None)
                        ...
                        st.session_state.nome_usuario = usuarios_para_listar[key_selecionada]["nome_exibicao"]
                        st.session_state.logado = True
                        st.session_state["tmp_logged_password"] = senha_salva
                        
                        # Fixa o cookie para as próximas abas reconhecerem sozinhas
                        cookie_val = f"{st.session_state.nome_usuario}|||{senha_salva}"
                        components.html(f"""
                        <script>
                        window.parent.document.cookie = "clicker_shared_session={cookie_val}; path=/; max-age=31536000; SameSite=Strict";
                        </script>
            ...
            if col_remover.button("Remover Dispositivo ❌", use_container_width=True):
                ...
                # Limpa o cookie local para esquecer a sessão
                components.html("""
                <script>
                window.parent.document.cookie = "clicker_shared_session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
                </script>
                """, height=0, width=0)
            ...
            st.info("Nenhuma conta salva detectada neste dispositivo. Faça login ou crie uma conta manualmente uma vez para ativar a sincronização automática.")
    ...
    with aba_registro:
        ...
        if st.button("Registrar Conta", use_container_width=True):
            ...
            if not reg_user or not reg_pass:
                ...
                # Salva no cookie compartilhado global do navegador
                cookie_val = f"{reg_user}|||{reg_pass}"
                components.html(f"""
                <script>
                window.parent.document.cookie = "clicker_shared_session={cookie_val}; path=/; max-age=31536000; SameSite=Strict";
                </script>
                ...
                st.success("Conta criada e guardada de forma permanente nas abas!")
                time.sleep(0.5)
                st.rerun()
...
with st.sidebar:
    ...
    st.caption("✓ Sessão sincronizada no dispositivo.")

    if st.button("Sair da Conta (Logout)", type="secondary", use_container_width=True):
        salvar_progresso_atual()
        
        # Destrói o cookie ao deslogar para que as abas limpem a memória
        components.html("""
        <script>
        window.parent.document.cookie = "clicker_shared_session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
        </script>
    ...
    if exibir_painel_dev:
        ...
        if placar_completo_dev:
            usuarios_dev_db = carregar_todos_usuarios()
            for i, jogador in enumerate(placar_completo_dev):
                name_jogador = jogador["Jogador"]
                key_jogador = name_jogador.lower()
                
                titulo_atual = ""
                if key_jogador in usuarios_dev_db:
                    titulo_atual = usuarios_dev_db[key_jogador]["dados"].get("titulo", "")
                ...
                if col_dev_pts.button("Pontos", key=f"dev_pts_{key_jogador}_{i}"):
                    if key_jogador in usuarios_dev_db:
                        usuarios_dev_db[key_jogador]["dados"]["pontos"] = max(0, usuarios_dev_db[key_jogador]["dados"].get("pontos", 0) + qtd_alteracao)
                        salvar_todos_usuarios(usuarios_dev_db)
                ...
                if col_dev_clk.button("Poder/C", key=f"dev_clk_{key_jogador}_{i}"):
                    if key_jogador in usuarios_dev_db:
                        usuarios_dev_db[key_jogador]["dados"]["poder_base"] = max(1, usuarios_dev_db[key_jogador]["dados"].get("poder_base", 1) + qtd_alteracao)
                        salvar_todos_usuarios(usuarios_dev_db)
                    if key_jogador == st.session_state.nome_usuario.lower():
                ...
                if col_dev_pps.button("Pontos/s", key=f"dev_pps_{key_jogador}_{i}"):
                    if key_jogador in usuarios_dev_db:
                        usuarios_dev_db[key_jogador]["dados"]["pontos_por_segundo"] = max(0, usuarios_dev_db[key_jogador]["dados"].get("pontos_por_segundo", 0) + qtd_alteracao)
                        salvar_todos_usuarios(usuarios_dev_db)
                    if key_jogador == st.session_state.nome_usuario.lower():
                ...
                with col_dev_t.popover("Title", use_container_width=True):
                    ...
                    if st.button("Aplicar", key=f"btn_dev_t_{key_jogador}_{i}", use_container_width=True):
                        if key_jogador in usuarios_dev_db:
                            ...
                            usuarios_dev_db[key_jogador]["dados"]["titulo"] = novo_t
                            salvar_todos_usuarios(usuarios_dev_db)
                            if key_jogador == st.session_state.nome_usuario.lower():
                ...
                if col_dev_ban.button("Ban", key=f"dev_ban_{key_jogador}_{i}"):
                    if key_jogador in usuarios_dev_db:
                        del usuarios_dev_db[key_jogador]
                        salvar_todos_usuarios(usuarios_dev_db)
...
if os.path.exists(LEADERBOARD_FILE):
    try:
        ...
        for jogador in dados_placar:
            nome = jogador["Jogador"]
            pontos = grandmother = jogador.get("Pontos", 0)
            if nome.lower() not in usuarios_unicos or pontos > usuarios_unicos[nome.lower()]["Pontos"]:
                usuarios_unicos[nome.lower()] = {"Jogador": nome, "Pontos": pontos}
