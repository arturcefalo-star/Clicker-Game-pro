...
def tem_titulo(titulo_necessario):
  if not st.session_state.get("logado") or not st.session_state.get(
      "nome_usuario"
  ):
    return False
...
def salvar_progresso_atual():
  if st.session_state.logado and st.session_state.nome_usuario:
    ...
    if username_key in usuarios and usuarios[username_key].get(
        "banido", False
    ):
      return
    ...
    if username_key in usuarios:
      ...
      atualizar_no_leaderboard(
          st.session_state.nome_usuario, st.session_state.pontos
      )
  ...
  if os.path.exists(LEADERBOARD_FILE):
    try:
      ...
      for jogador in dados:
        ...
        if (
            nome.lower() not in usuarios_unicos
            or pontos > usuarios_unicos[nome.lower()]["Pontos"]
        ):
          usuarios_unicos[nome.lower()] = {"Jogador": nome, "Pontos": pontos}

      return sorted(
          usuarios_unicos.values(), key=lambda x: x["Pontos"], reverse=True
      )[:5]
    except Exception:
      return []
  return []
...
def carregar_configuracoes_globais():
  if os.path.exists(AVISOS_FILE):
    try:
      ...
      return {
          "mensagem": "",
          "multiplicador_evento": 1,
          "multiplicador_sorte": 1,
      }
...
bridge_data = st.text_input(
    "bridge_storage_input",
    key="bridge_storage_input",
...
if not st.session_state.logado:
  ...
  aba_login, aba_salvas, aba_registro = st.tabs(
      ["Entrar na Conta", "Contas Já Criadas 💾", "Criar Nova Conta"]
  )
      ...
      if user_key in usuarios and usuarios[user_key]["senha"] == log_pass:
        if usuarios[user_key].get("banido", False):
          st.error(
              ...
              f" {usuarios[user_key].get('motivo_ban', 'Não informado')}"
          )
        else:
...
if user_key_atual in usuarios_verificacao and usuarios_verificacao[
    user_key_atual
].get("banido", False):
  motivo_banimento = usuarios_verificacao[user_key_atual].get(
      "motivo_ban", "Sem motivo especificado."
  )

  st.error("🚫 **SUA CONTA FOI SUSPENSA**")
  st.warning(
      "Você foi banido por um administrador pelo seguinte"
      f" motivo:\n\n> **{motivo_banimento}**"
  )
  st.info(
      "Caso acredite que isso foi um engano, entre em contato com a"
      " administração."
  )
...
def atualizar_poder_clique():
  ...
  if st.session_state.totem_equipado:
    poder_calculado = int(
        poder_calculado * st.session_state.totem_equipado["multiplicador"]
    )
...
def renderizar_area_clique():
  ...
  if st.session_state.nome_usuario.lower() in chk_users and chk_users[
      st.session_state.nome_usuario.lower()
  ].get("banido", False):
    st.rerun()
  ...
  if st.session_state.totem_equipado:
    st.write(
        f"🌟 **Totem Ativo:** {st.session_state.totem_equipado['nome']}"
        f" ({st.session_state.totem_equipado['multiplicador']}x Bônus)"
    )
  ...
  if st.session_state.mundo_atual == 2:
    if st.button(
      ...
      salvar_progresso_atual()
    st.write(
        f"**Poder de clique:** {(st.session_state.poder_clique * 2):,} (2X do"
        " Mundo)"
    )
  else:
    if st.button(
...
with st.sidebar:
  prefixo_exibicao = (
      f"[{st.session_state.titulo}] " if st.session_state.titulo else ""
  )
  st.write(
      f"Conectado como: **{prefixo_exibicao}{st.session_state.nome_usuario}**"
  )
  ...
  if st.button(
      "Sair da Conta (Logout)", type="secondary", use_container_width=True
  ):
    salvar_progresso_atual()
    resetar_estados_jogador_local()
    st.rerun()
  ...
  if exibir_painel_dev:
    ...
    if usuarios_db_dev:
      for i, (key_jogador, dados_user) in enumerate(
          list(usuarios_db_dev.items())
      ):
        ...
        col_dev_pts, col_dev_clk, col_dev_pps, col_dev_t, col_dev_ban = (
            st.columns([1, 1, 1, 1.2, 1])
        )
        ...
        with col_dev_t.popover("Title", use_container_width=True):
          ...
          try:
            idx_atual = (
                opcoes_titulos_dev.index(titulo_atual)
                if titulo_atual in opcoes_titulos_dev
                else 0
            )
          except ValueError:
            idx_atual = 0
          opcao_titulo = st.selectbox(
              "Escolha:",
              opcoes_titulos_dev,
              key=f"sel_dev_t_{key_jogador}_{i}",
              index=idx_atual,
          )
          if st.button(
              "Aplicar",
              key=f"btn_dev_t_{key_jogador}_{i}",
              use_container_width=True,
          ):
        ...
        with col_dev_ban.popover(
            popover_label, use_container_width=True, key=popover_key
        ):
          if esta_banido:
            st.write("Clique abaixo para desbanir:")
            if st.button(
                "Confirmar Desban",
                key=f"unban_confirm_{key_jogador}_{i}",
                type="primary",
                use_container_width=True,
            ):
            ...
            motivo_input = st.text_input(
                "Motivo:",
                value="Uso de hacks/bugs",
                key=f"motivo_ban_{key_jogador}_{i}",
            )
            if st.button(
                "Confirmar Ban",
                key=f"ban_confirm_{key_jogador}_{i}",
                type="primary",
                use_container_width=True,
            ):
    ...
    mapeamento_jogadores = {
        usuarios_db_inspect[k]["nome_exibicao"]: k
        for k in usuarios_db_inspect
        if "nome_exibicao" in usuarios_db_inspect[k]
    }
    lista_jogadores = list(mapeamento_jogadores.keys())

    if lista_jogadores:
      jogador_selecionado = st.selectbox(
          "Selecione um jogador do banco de dados:",
          lista_jogadores,
          key="inspect_select",
      )
      if st.button(
        ...
        col_ins3.metric(
            "Pontos/Seg", f"{dados_player.get('pontos_por_segundo', 0):,}"
        )

        mundo_txt = (
            "Mundo 2" if dados_player.get("mundo_atual", 1) == 2 else "Mundo 1"
        )
        m2_liberado = (
            "Sim"
            if dados_player.get("mundo_2_desbloqueado", False)
            else "Não"
        )
        st.write(
            f" **Mundo Atual:** {mundo_txt} |  **Mundo 2 Desbloqueado?**"
            f" {m2_liberado}"
        )
        ...
        with col_p1:
          ...
          st.write(
              f"Slot 1: {p1['nome']} (+{p1['bonus']:,})"
              if p1
              else "Slot 1: Vazio"
          )
          st.write(
              f"Slot 2: {p2['nome']} (+{p2['bonus']:,})"
              if p2
              else "Slot 2: Vazio"
          )
        with col_p2:
          ...
          st.write(
              f"Slot 1: {pm1['nome']} (+{pm1['bonus']:,})"
              if pm1
              else "Slot 1: Vazio"
          )
          st.write(
              f"Slot 2: {pm2['nome']} (+{pm2['bonus']:,})"
              if pm2
              else "Slot 2: Vazio"
          )
        ...
        st.write(
            f"{tot_player['nome']} ({tot_player['multiplicador']}x)"
            if tot_player
            else "Nenhum Totem Equipado"
        )
  ...
  if acesso_admin:
    st.success(
        f"Acesso automático concedido via título [{st.session_state.titulo}]!"
    )
    exibir_painel_admin = True
  else:
    if st.checkbox("Ativar Modo Administrador"):
      senha_input = st.text_input(
          "Digite a senha de Admin:", type="password", key="senha_admin_input"
      )
  ...
  if exibir_painel_admin:
    ...
    qtd_pontos = st.number_input(
        "Quantidade de pontos para Add/Rem:",
        min_value=1,
        value=1000,
        step=100,
        key="admin_pt_val",
    ...
    if usuarios_db:
      for i, (key_jogador, dados_user) in enumerate(
        ...
        if col_adm3.button("Add", key=f"add_{key_jogador}_{i}"):
          usuarios_db[key_jogador]["dados"]["pontos"] = max(
              0,
              usuarios_db[key_jogador]["dados"].get("pontos", 0) + qtd_pontos,
          )
          ...
          if key_jogador == st.session_state.nome_usuario.lower():
            ...
            st.session_state.pontos_leaderboard_cache = st.session_state.pontos
          atualizar_no_leaderboard(
              name_jogador, usuarios_db[key_jogador]["dados"]["pontos"]
          )
          st.rerun()
        ...
        if col_adm4.button("Rem", key=f"rem_{key_jogador}_{i}"):
          ...
          salvar_todos_usuarios(usuarios_db)
          if key_jogador == st.session_state.nome_usuario.lower():
            st.session_state.pontos = max(
                0, st.session_state.pontos - qtd_pontos
            )
            st.session_state.pontos_leaderboard_cache = st.session_state.pontos
          atualizar_no_leaderboard(
              name_jogador, usuarios_db[key_jogador]["dados"]["pontos"]
    ...
    if col_msg1.button(
        "Enviar Mensagem", use_container_width=True, key="admin_send_msg"
    ):
    ...
    status_evento = (
        f"ATIVADO ({mult_evento}X)" if mult_evento > 1 else "DESATIVADO"
    )
    ...
    if st.button(
        ...
        type="secondary",
        use_container_width=True,
        disabled=(mult_evento == 1),
        key="btn_desativar_evento",
    ):
    ...
    status_sorte = (
        f"ATIVADO ({mult_sorte}X)" if mult_sorte > 1 else "DESATIVADO"
    )
    ...
    if st.button(
        ...
        type="secondary",
        use_container_width=True,
        disabled=(mult_sorte == 1),
        key="btn_desativar_sorte",
    ):
  ...
  if acesso_apoiador:
    ...
    if st.checkbox("Ativar Modo Apoiador"):
      senha_apoio_input = st.text_input(
          "Digite a senha de Apoiador:", type="password", key="senha_apoio_input"
      )
  ...
  if exibir_painel_apoiador:
    ...
    qtd_pontos_apoio = st.number_input(
        "Quantidade de pontos para Add/Rem:",
        min_value=1,
        value=50000,
        step=1000,
        key="qtd_pontos_apoio",
    )

    col_apoio1, col_apoio2, col_apoio3 = st.columns([2, 1, 1])
    col_apoio1.write(
        f"**Você {st.session_state.nome_usuario}**: {st.session_state.pontos:,}"
        " pts"
    )

    if col_apoio2.button(
        "Add", key="add_pontos_apoio", use_container_width=True
    ):
      usuarios_db = carregar_todos_usuarios()
      ...
      if key_jogador in usuarios_db:
        usuarios_db[key_jogador]["dados"]["pontos"] = max(
            0,
            usuarios_db[key_jogador]["dados"].get("pontos", 0)
            + qtd_pontos_apoio,
        )
        salvar_todos_usuarios(usuarios_db)
    ...
    if col_apoio3.button(
        "Rem", key="rem_pontos_apoio", use_container_width=True
    ):
      usuarios_db = carregar_todos_usuarios()
      key_jogador = st.session_state.nome_usuario.lower()
      if key_jogador in usuarios_db:
        ...
        salvar_todos_usuarios(usuarios_db)
      st.session_state.pontos = max(
          0, st.session_state.pontos - qtd_pontos_apoio
      )
      st.session_state.pontos_leaderboard_cache = st.session_state.pontos
      atualizar_no_leaderboard(
          st.session_state.nome_usuario, st.session_state.pontos
...
if mult_evento > 1:
  st.warning(
      f" **EVENTO GLOBAL ATIVO:** Cliques concedendo **{mult_evento}X** de"
      " Pontos!"
  )

if mult_sorte > 1:
  st.success(
      " 🍀 **EVENTO DE SORTE ATIVO:** Pets Raros (<=15%) com"
      f" **{mult_sorte}X** mais chance!"
  )
...
if not st.session_state.mundo_2_desbloqueado:
  ...
  if st.button(
      f"Comprar Mundo 2 (Custo: {CUSTO_MUNDO_2:,} Pts)",
      disabled=desativar_compra_mundo,
      use_container_width=True,
  ):
  ...
  if st.session_state.mundo_atual == 1:
    if st.button(
        "Teleportar para mundo 2", type="primary", use_container_width=True
    ):
      st.session_state.mundo_atual = 2
      salvar_progresso_atual()
      st.rerun()
    ...
    if st.button(
        "Voltar para mundo 1", type="secondary", use_container_width=True
    ):
      st.session_state.mundo_atual = 1
      salvar_progresso_atual()
      st.rerun()
...
if st.session_state.mundo_atual == 2:
  ...
  with col_m2_egg1:
    ...
    if st.button(
        f"Abrir Ovo = {CUSTO_OVO_MUNDO_2_BARATO:,} Pontos",
        disabled=desativar_m2_ovo1,
        key="botao_m2_ovo1",
    ):
      if st.session_state.pontos >= CUSTO_OVO_MUNDO_2_BARATO:
        ...
        sorteado = random.choices(
            ...
            weights=[ch1_m2_o1, ch2_m2_o1, ch3_m2_o1],
            k=1,
        )[0]
  ...
  with col_m2_egg2:
    ...
    st.write(
        f"{NOME_PET_M2_R3}: **{ch3_m2_o2:.1f}%** (+{BONUS_PET_M2_R3:,} Pts) 🍀"
    )

    desativar_m2_ovo2 = st.session_state.pontos < CUSTO_OVO_MUNDO_2_CARO
    if st.button(
        f"Abrir Ovo = {CUSTO_OVO_MUNDO_2_CARO:,} Pontos",
        disabled=desativar_m2_ovo2,
        key="botao_m2_ovo2",
    ):
            ...
            [
                {
                    "nome": NOME_PET_M2_R1,
                    "arquivo": "logo10.png",
                    "bonus": BONUS_PET_M2_R1,
                    "chance": "50%",
                },
                {
                    "nome": NOME_PET_M2_R2,
                    "arquivo": "logo11.png",
                    "bonus": BONUS_PET_M2_R2,
                    "chance": "35%",
                },
                {
                    "nome": NOME_PET_M2_R3,
                    "arquivo": "logo12.png",
                    "bonus": BONUS_PET_M2_R3,
                    "chance": "15%",
                },
            ],
            weights=[ch1_m2_o2, ch2_m2_o2, ch3_m2_o2],
            k=1,
        )[0]
  ...
  with col3:
    ...
    if st.button(
        f"Abrir Ovo = {custo_ovo1} Pontos",
        disabled=desativar_ovo1,
        key="botao_ovo1",
    ):
      if st.session_state.pontos >= custo_ovo1:
        ...
        sorteado_ovo1 = random.choices(
            ...
            weights=[ch1_m1_o1, ch2_m1_o1, ch3_m1_o1],
            k=1,
        )[0]
  ...
  with col4:
    ...
    if st.button(
        f"Abrir Ovo = {custo_ovo2} Pontos",
        disabled=desativar_ovo2,
        key="botao_ovo2",
    ):
      if st.session_state.pontos >= custo_ovo2:
        ...
        sorteado_ovo2 = random.choices(
            ...
            weights=[ch1_m1_o2, ch2_m1_o2, ch3_m1_o2],
            k=1,
        )[0]
...
def renderizar_loja_totens():
  ...
  st.caption(
      "⏳ Os itens da loja mudam em:"
      f" **{int(tempo_restante_segundos/60)}m {tempo_restante_segundos%60}s**"
  )
...
if st.session_state.mundo_atual == 2:
  melhorias_clique = [
      ...
      {"qtd": 2500000, "custo": 3500000000},
      {"qtd": 5000000, "custo": 8000000000},
      {"qtd": 10000000, "custo": 20000000000},
      {"qtd": 25000000, "custo": 50000000000},
      {"qtd": 50000000, "custo": 100000000000},
  ]
  melhorias_passivas = [
  ...
  melhorias_passivas = [
      ...
      {"qtd": 2000, "custo": 145000},
      {"qtd": 5000, "custo": 360000},
      {"qtd": 10000, "custo": 725000},
...
with col1:
  ...
  with st.container(height=350):
    for i, item in enumerate(melhorias_clique):
      ...
      if st.button(
          texto, key=key_btn, disabled=desativado, use_container_width=True
      ):
...
if os.path.exists(LEADERBOARD_FILE):
  try:
    ...
    if placar_final:
      dados_formatados = [
          {"Jogador": j["Jogador"], "Pontos": f"{j['Pontos']:,}"}
          for j in placar_final
      ]
...
if not st.session_state.confirmando_reset:
  ...
  st.warning(
      "⚠️ **Você tem certeza absoluta?** Isso apagará permanentemente todos os"
      " seus pontos!"
  )
  ...
  with col_sim:
    if st.button("SIM, deletar tudo", type="primary", use_container_width=True):
      ...
      st.components.v1.html(
          f"""<script>window.parent.postMessage({{type: "REMOVE_ACCOUNT", user: "{user_key}"}}, "*");</script>""",
          height=0,
          width=0,
      )
      resetar_estados_jogador_local()
